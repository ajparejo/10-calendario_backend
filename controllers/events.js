    ...
    try {
        ...
        if(evento.user.toString() !== uid){
            return res.status(401).json({
                ok: false,
                msg: 'no puede modificar el evento'
            })
        }
        const nuevoEvento = {
            ...req.body,
...
const borrarEventos = async (req, res = response) => {
    ...
    try {
        ...
        if(evento.user.toString() !== uid){
            return res.status(401).json({
                ok: false,
                msg: 'no puede eliminar el evento'
            })
        }
...
module.exports = { obtenerEventos, crearEvento, actualizarEventos, borrarEventos }